---
title: TODO 汇总
outline: [2, 3]
---

# TODO 汇总

> 最后更新时间: 2026-07-16 15:38:09 (UTC+8)

共找到 36 个 TODO 项

## beyond/abroad/phd.md

- **第 11 行**: TODO: 境外申请直博及其他博士项目相关
  - [查看源文件](/en/beyond/abroad/phd.md)

## beyond/mainland/phd.md

- **第 11 行**: TODO: 申请境内高校博士项目相关
  - [查看源文件](/en/beyond/mainland/phd.md)

## beyond/mainland/recommend_graduate.md

- **第 11 行**: TODO: 保研政策、经验等等
  - [查看源文件](/en/beyond/mainland/recommend_graduate.md)

## beyond/mainland/unified_admission.md

- **第 11 行**: TODO: 考研相关内容
  - [查看源文件](/en/beyond/mainland/unified_admission.md)

## copyrights.md

- **第 22 行**: TODO: 对齐仓库根目录 `LICENSE`、首页 footer 中的 `CC-BY-SA` 文案、`others/ask-the-smart-way.md` 的 `CC0` 说明，以及其他单页例外声明，给出统一且可执行的授权口径。
  - [查看源文件](/en/copyrights.md)
- **第 24 行**: TODO: 为第三方素材补充来源、授权方式和复用限制，尤其是图片、PDF、嵌入组件、App 相关资源和外部网页内容。
  - [查看源文件](/en/copyrights.md)
- **第 26 行**: TODO: 明确贡献者提交文本、图片、表格和脚本时默认采用什么授权，以及维护者在合并前需要检查哪些版权信息。
  - [查看源文件](/en/copyrights.md)

## en/copyrights.md

- **第 22 行**: TODO: Align the repository root `LICENSE`, the homepage footer text that says `CC-BY-SA`, the `CC0` note in `others/ask-the-smart-way.md`, and any other page-level exception statements.
  - [查看源文件](/en/copyrights.md)
- **第 24 行**: TODO: Add source, license, and reuse notes for third-party assets, especially images, PDFs, embedded components, app-related resources, and mirrored external content.
  - [查看源文件](/en/copyrights.md)
- **第 26 行**: TODO: Document the default contribution license for text, images, tables, and scripts, plus the copyright checks maintainers should perform before merging.
  - [查看源文件](/en/copyrights.md)

## en/get-started.md

- **第 97 行**: TODO: Replace this Chinese draft with a real English onboarding page, and keep year-specific logistics synchronized with the Chinese source page each admission cycle.
  - [查看源文件](/en/get-started.md)

## en/introduction.md

- **第 27 行**: TODO: Rewrite this page as a handbook introduction for readers and contributors, instead of keeping it as a generic university encyclopedia summary.
  - [查看源文件](/en/introduction.md)

## get-started.md

- **第 99 行**: TODO: 重写2025版本的迎新指南
  - [查看源文件](/en/get-started.md)

## health/alive_first.md

- **第 11 行**: TODO: 生命观、日常安全（交通、反诈）、心理健康援助、灾害应急指南
  - [查看源文件](/en/health/alive_first.md)

## infra/gzic/map.md

- **第 10 行**: TODO: 国际基础设施介绍
  - [查看源文件](/en/infra/gzic/map.md)

## infra/gzic/nearby.md

- **第 7 行**: TODO: 国际周边的简介页面
  - [查看源文件](/en/infra/gzic/nearby.md)

## infra/hemc/nearby.md

- **第 9 行**: TODO: 大学城周边
  - [查看源文件](/en/infra/hemc/nearby.md)

## infra/hemc/suishi.md

- **第 9 行**: TODO: 碎石简介尚未编写
  - [查看源文件](/en/infra/hemc/suishi.md)

## infra/wushan/map.md

- **第 7 行**: TODO: 五山校区基础设施的概览页面
  - [查看源文件](/en/infra/wushan/map.md)

## infra/wushan/nearby.md

- **第 7 行**: TODO: 五山校区周边去处的简介页面
  - [查看源文件](/en/infra/wushan/nearby.md)

## introduction.md

- **第 46 行**: - 阅读[贡献指南](/en/others/contributing.html)与[TODO 汇总](/en/others/todo.html)，认领尚未完善的内容。
  - [查看源文件](/en/introduction.md)

## learn/curricular/common_basic_lessons.md

- **第 76 行**: TODO: 完善线代的课程描述与学习资料
  - [查看源文件](/en/learn/curricular/common_basic_lessons.md)
- **第 87 行**: TODO: 完善概率论的课程描述与学习资料
  - [查看源文件](/en/learn/curricular/common_basic_lessons.md)
- **第 93 行**: TODO: 完善复变的课程描述与学习资料
  - [查看源文件](/en/learn/curricular/common_basic_lessons.md)

## learn/curricular/exam.md

- **第 9 行**: TODO: 华南理工大学与考试相关的规章制度，及应对考试可供参考的一些告诫
  - [查看源文件](/en/learn/curricular/exam.md)

## learn/curricular/majors.md

- **第 11 行**: TODO: 其他学院的专业学习攻略
  - [查看源文件](/en/learn/curricular/majors.md)

## learn/extra/contest.md

- **第 26 行**: TODO: 后续可将竞赛目录进一步结构化，区分“校内创新分导向”“学科竞赛导向”“就业技能导向”三类入口。
  - [查看源文件](/en/learn/extra/contest.md)
- **第 28 行**: TODO: 需要进一步完善，添加更多竞赛内容的指引
  - [查看源文件](/en/learn/extra/contest.md)

## life/eat/gzic.md

- **第 11 行**: TODO: 广州国际校区的觅食指南
  - [查看源文件](/en/life/eat/gzic.md)

## life/entertainment/hemc_gzic.md

- **第 11 行**: TODO: 大学城校区与广州国际校区周边商圈及娱乐设施，公园啥的我觉得也可以写写
  - [查看源文件](/en/life/entertainment/hemc_gzic.md)

## life/entertainment/wushan.md

- **第 11 行**: TODO: 五山校区周边的娱乐去处
  - [查看源文件](/en/life/entertainment/wushan.md)

## others/app.md

- **第 9 行**: TODO: 补充 App 的能力边界、平台支持矩阵、版本来源和已知限制说明，避免页面只剩下载入口。
  - [查看源文件](/en/others/app.md)
- **第 11 行**: TODO: 跟进 Issue #23，明确课表导入是否支持 WebVPN 链接、用户自定义 URL，还是两者都支持。
  - [查看源文件](/en/others/app.md)

## others/contributing.md

- **第 9 行**: 可加入编辑部提交贡献文档，或直接为本项目提交PR来贡献内容！在开始前，建议先阅读[维护路线图](/en/others/roadmap.html)和[TODO 汇总](/en/others/todo.html)，这样更容易把贡献投到高优先级缺口上。
  - [查看源文件](/en/others/contributing.md)
- **第 174 行**: TODO: 补充文档编撰规范（附件命名、来源标注、外链规范、时间敏感信息标记、翻译同步规则、引用官方信息的写法）。
  - [查看源文件](/en/others/contributing.md)
- **第 176 行**: TODO: 建立年度更新流程（迎新、校巴、地图、转专业、校医院、周边信息等高时效页面每学年至少核对一次）。
  - [查看源文件](/en/others/contributing.md)
