...
- Custom code lives in `docs/.vitepress/` (config, theme, plugins) and `scripts/`.
- `docs/.vuepress/` is legacy VuePress config in the process of being removed. Do not add new code there; migrate any remaining dependencies to `docs/.vitepress/`.
- `docs/public/` is the single canonical static assets directory. `docs/.vuepress/public/` is legacy and should be removed once all consumers are migrated.
...
- Custom VitePress plugins: `docs/.vitepress/plugins/`, `docs/.vitepress/composables/`
- Static assets: `docs/public/` (VitePress public directory — the canonical source)
- Content pages (zh): `docs/**/*.md`
- Content pages (en): `docs/en/**/*.md`
- Scripts: `scripts/`
- Legacy (being removed — do not add new code here): `docs/.vuepress/`
...
### English locale and LLM translation metadata

- English content lives under `docs/en/` mirroring the `docs/` structure.
- This project is committed to **real, human-quality English content**, not placeholder or machine-only output.
- When an agent (LLM) translates a page from Chinese to English, add `llm_translated: true` to the English page's frontmatter metadata. This allows maintainers to identify AI-assisted translations for later human review.
- Example frontmatter for a translated English page:
  ```yaml
  ---
  lang: en-US
  title: Getting Started
  description: A guide for new students
  llm_translated: true
  ---
  ```
- Do NOT add `llm_translated: true` when a human writes or edits English content directly.
- All English pages must have `lang: en-US` (not `lang: zh-CN`), English `title` and `description`, and English body content.
...
- `docs/public/` is the canonical static assets directory. Do not duplicate assets in `docs/.vuepress/public/` — that directory is legacy and targeted for removal.
